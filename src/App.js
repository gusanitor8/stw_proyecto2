...
function App() {
  ...
  async function fetchData() {
    const apiUrl = `https://api.weatherbit.io/v2.0/current?lat=${latitude}&lon=${longitude}&include=uv_index&key=e3ba8c8239954b15aa6fa94f749125f0`; 
    console.log(apiUrl);   
    try {
  ...
  const fetchCityWeather = async () => {
    const apiUrl = `https://api.weatherbit.io/v2.0/current?city=${city}&include=uv_index&key=e3ba8c8239954b15aa6fa94f749125f0`;
    try {
      const response = await fetch(apiUrl);
      if (!response.ok) {
        throw new Error("API request failed");
