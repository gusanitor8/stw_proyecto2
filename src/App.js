...
function App() {
  ...
  async function fetchData() {
    const apiUrl = `https://api.weatherbit.io/v2.0/current?lat=${latitude}&lon=${longitude}&include=uv_index&key=${process.env.REACT_APP_API_KEY}`; 
    console.log(apiUrl);   
    try {
  ...
  const fetchCityWeather = async () => {
    const apiUrl = `https://api.weatherbit.io/v2.0/current?city=${city}&include=uv_index&key=${process.env.REACT_APP_API_KEY}`;
    try {
      const response = await fetch(apiUrl);
      if (!response.ok) {
        throw new Error("API request failed");
